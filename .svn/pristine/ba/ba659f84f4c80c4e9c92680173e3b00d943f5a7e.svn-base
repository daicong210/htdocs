  ...
  def column_value(column, issue, value)
    ...
    when 'String'
      if column.name == :subject
      	@txt = h(value)
        if issue.due_date && format_date(issue.due_date)<Time.now.strftime("%Y-%m-%d") && issue.status_id!=3 && issue.status_id!=5 && issue.status_id!=6 
          @txt = "<font color=\"#FF0000\">(delayed)"+h(value)+"</font>"
        end
        link_to(@txt.html_safe, :controller => 'issues', :action => 'show', :id => issue)
