...
module ApplicationHelper
  ...
  def link_to_issue(issue, options={})
    ...
    s = link_to text, issue_path(issue), :class => issue.css_classes + c, :title => title
    s << " "
    s << avatar(issue.assigned_to, :size => "14").to_s.html_safe + (issue.assigned_to ? link_to_user(issue.assigned_to) : "-")
    s << h(" #{subject}") if subject
    s = h("#{issue.project} - ") + s if options[:project]
	  s << link_to_issue_detail(issue)
    s
  end

  def link_to_issue_detail(issue)
    s = "(#{h(issue.status.name)},"
    #s = s + l(:field_assigned_to) + ":" + avatar(issue.assigned_to, :size => "14").to_s.html_safe + (issue.assigned_to ? link_to_user(issue.assigned_to) : "-")
    s ="#{s}#{link_to_issue_customed(issue, 1)}"
    s ="#{s},#{link_to_issue_journal(issue)})"
    s.html_safe
  end

  def link_to_issue_journal(issue, no_html=false, options={})
	  s = ''
        ...
        journal.details.each do |detail|
          case detail.property
            when 'attr'
              ...
              when 'status_id'
                if detail.value=="8"
                  value = link_to_issue_show_detail(field, detail.value)
                  old_value = link_to_issue_show_detail(field, detail.old_value)

                  s << format_date(journal.created_on)
                  s << " "
                  s << l(:button_submit)
                  s << value
                  break
  ...
  def link_to_issue_show_detail(field, id)
  ...
  def link_to_issue_journal_show_detail(detail, no_html=false, options={})
    multiple = false
    
  end

  def link_to_issue_customed(issue, index)
    ...
    ordered_values.compact.each do |value|
	  if value.custom_field.id==8 && !value.value.empty?
      s << "#{ h(value.custom_field.name) }:"
      s << "#{ simple_format_without_paragraph(h(show_value(value))) }"
      n += 1
	  break
	  end
    end
  ...
  def link_to_issue_assigned_to(collection, selected)
	s = ''
    collection.sort.each do |element|
	  if option_value_selected?(element, selected)	
        s << %(#{h element.name} )
	  end
    end
    s.html_safe
  ...
  def departments_check_box_tags(name, departments)
    s = ''
    departments.sort.each do |department|
      s << "<label>#{ check_box_tag name, department.id, true } #{department.name}</label>\n"
    end
    s.html_safe
  end
  ...
  def leader_image(leader=false)
    if leader
      image_tag 'leader.png'
    else
      image_tag 'normal-user.png'
    end
  end
  ...
  # 下面的代码是用于处理发送RTX提醒的
  # 传入Issue对象
  def notify_rtx(issue, cur_user_id)
    if !issue || !issue.assigned_to_id || !cur_user_id || cur_user_id==!issue.assigned_to_id || issue.status_id==3 || issue.status_id==5
      return
    end

    rtxname = get_user_name(issue, issue.assigned_to_id)
    opname = get_user_name(issue, cur_user_id)
    ...
    params={}
    #params["UserNames"] = l("rtx.pengzhifeng")  + "," + rtxname
	notes = ""
	notes = notes.html_safe	if issue.notes
    params["UserNames"] = rtxname
    params["msg"] =  l(:"rtx.msg", :id => issue.id, :opname => opname, :notes => notes, :subject => issue.subject)
    # UserNames=姓名1,姓名2&msg=内容
    uri = "http://172.16.0.3/rtxalarm/rtxmsg.aspx?UserNames=" + params["UserNames"] + "&msg=" + params["msg"]
     
    html_response = nil  
    logger.error(uri) if logger
    #open(URI::encode( u2g(uri) ))

    #logger.error(html_response) if logger   
  end
  ...
  def get_user_name(issue, user_id)
    if !issue || !user_id
      return nil
    end
    rtxname = nil
    case user_id
      when 3 #peng zhifeng 
        rtxname = l("rtx.pengzhifeng")
      when 10 #pin zhi
        rtxname = l("rtx.pinzhi")
      when 9 #kai fa
        rtxname = l("rtx.kaifa")
      else
        issue.project.users.each do |value|
          if value["id"]==user_id
            rtxname = value.to_s.gsub(" ", "")
            break
          end
        end
    end
  ...
  #获取当前用户某个项目下的所属部门
  def get_current_user_department(project_id)
    current_user = User.current
    if !project_id.blank? && !current_user.blank?
      DepartmentMember.find_by_sql("select * from department_members where department_id in(select id from departments where project_id = #{project_id}) and user_id = #{current_user.id}")
    end
  end
  ...
  # 截取描述
  def get_text_limit(str,len=0)
    if str.blank? or len.blank? or !len.is_a?(Numeric)
      return ""
    end
    if str.length>len and len !=0
       "#{str[0,len]}..."
    else
      str.gsub("\n","<br>")
    end
