...
module CustomFieldsHelper
  ...
  def custom_field_tag(name, custom_value)	
    ...
    when "list"
	  title = h(custom_value.custom_field.name)
	  if custom_field.id==8 then
	    s = custom_field_checkboxes(name, custom_value)
		s.html_safe
	  else
  ...
  def custom_field_checkboxes(name, custom_value, checked=nil)
	custom_field = custom_value.custom_field
    field_name = "#{name}[custom_field_values][#{custom_field.id}]"
    field_name << "[]" if custom_field.multiple?
    field_id = "#{name}_custom_field_values_#{custom_field.id}"
	...
	s = custom_field.possible_values_options(custom_value.customized)
	re = ""
	for k in s 
      c = checked.nil? ? custom_field_checkboxes_ischecked(k[1], custom_value.value) : checked
      #tag = check_box_tag field_name, k[2], c, :id => nil
	  tag = check_box_tag(field_name, k[1], c, tag_options)
	  o = content_tag 'label', "#{tag} #{k[0]}".html_safe,
                  :id => field_id,
				  :class => "floating"
  ...
  def custom_field_checkboxes_ischecked(v, c)
	for i in c 
		if i==v then
			return true
		end
