...
class Principal < ActiveRecord::Base
  ...
  # Principals that are members of projects and departments
  scope :not_member_of_department, lambda {|projects,department|
                        projects = [projects] unless projects.is_a?(Array)
                        if projects.empty?
                          where("1=0")
                        else
                          ids = projects.map(&:id)
                          sql1 = "#{Principal.table_name}.id IN (SELECT DISTINCT user_id FROM #{Member.table_name} WHERE project_id IN (?)))"
                          sql2 = "(#{Principal.table_name}.id NOT IN (SELECT DISTINCT user_id FROM #{DepartmentMember.table_name} where department_id =#{department.id})"
                          where("#{sql1} AND #{sql2}", ids)
                        end
                      }
